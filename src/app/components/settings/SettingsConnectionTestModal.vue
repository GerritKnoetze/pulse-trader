...
interface Step {
  ...
  label:  string
  status: StepStatus
  logs:   string[]          // all messages logged for this step
}
...
const steps = ref<Step[]>([
  ...
  { id: 'api',    label: 'REST API Test',       status: 'pending', logs: [] },
  { id: 'ws',     label: 'WebSocket Test',       status: 'pending', logs: [] },
])
...
function runTest() {
  ...
  const es = new EventSource('/api/settings/connection-test')
...
// Start test automatically when modal opens
  ...
  <Teleport to="body">
    <div v-if="open" class="ct-overlay" @click.self="done && emit('close')">
      <div class="ct-modal">
        ...
        <!-- Header -->
        <div class="ct-header">
          <span class="ct-title">Connection Test — Massive.com</span>
        ...
        <div v-if="done" class="ct-result" :class="`ct-result--${overall}`">
          ...
          <span v-if="overall === 'success'">All tests passed — Massive.com is configured correctly</span>
